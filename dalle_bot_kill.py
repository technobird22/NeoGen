...
def init_discord_bot():
    ...
    async def on_message(message):
        global history, seed, is_generating, to_notify, do_gen, instances
        ...
        if message.content.startswith('.dalle'):
            ...
            # Batch size
            batch_size = argument_parser('n', prompt, jax.device_count()*2)
            ...
            try:
                ...
                await message.reply("⚠ temperature, and conditional scale must be valid floats. Top P and Top K must be integers")
                await waiting.delete()
                return
            ...
            # await message.channel.send(f"**DEBUG** <@{presets.OWNER_ID}>: `{filenames}`")
